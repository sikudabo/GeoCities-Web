...
router.route('/api/get-post/:_id').get(async (req, res) => {
    ...
    try {
        ...
        if (!post) {
            res.status(200).json({ isError: false, isPostDeleted: true, message: 'This post has been deleted', post });
            return;
        }

        res.status(200).json({ isError: false, isPostDeleted: false, post });
        return;
    } catch(err) {
