...
router.route('/api/follow-unfollow-user').post(async (req, res) => {
    const { _id, followerId, isFollow } = req.body; 

    try {
        if (isFollow) {
            await UserModel.updateOne({ _id }, { $push: { followers: followerId }});
            await UserModel.updateOne({ _id: followerId }, { $push: { following: _id }});
        ...
        await UserModel.updateOne({ _id: followerId }, { $pull: { following: _id }});
